...
``ANYsolver>=0.4.6,<0.5``, ``ANYgeometry>=0.4.3,<0.5``,
``ANYmaterial>=0.2.0,<0.3``, ``ANYmesher>=0.4.0,<0.6``,
``ANYfileio>=0.3.2,<0.4``, ``ANYbuckling>=0.1.1,<0.2``,
``ANY3dView>=0.5.5,<0.6``, and ``ANYtk3D>=0.5.5,<0.6``. Install
``ANYstructure[gpu]`` to add the optional ModernGL backend.
For coordinated sibling-checkout development, install the source projects
below into the same environment.
...
ANYmesher in ``>=0.4.0,<0.6``. Set ``ANYSTRUCTURE_ANYMESHER_ROOT`` to another
