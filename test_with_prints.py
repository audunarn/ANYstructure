#!/usr/bin/env python
"""
Test with debug prints to see what's happening.
"""
...
class Camera3D:
    ...
    def _update_view_matrix(self):
        ...
        print(f"Camera position: ({self.position.x:.2f}, {self.position.y:.2f}, {self.position.z:.2f})")
    ...
    def project_point(self, point, width, height):
        ...
        print(f"  Point ({point.x:.1f}, {point.y:.1f}, {point.z:.1f}) -> cam ({cam_x:.1f}, {cam_y:.1f}, {cam_z:.1f}, {cam_w:.1f})")
        
        if cam_z <= self.near:
            print(f"    -> BEHIND CAMERA (z={cam_z:.1f} <= near={self.near})")
            return None
    ...
    def draw_cylinder(self):
        ...
        for i in range(segments):
            ...
            if s1 and s2:
                ...
                print(f"  Drew line from ({s1[0]:.1f}, {s1[1]:.1f}) to ({s2[0]:.1f}, {s2[1]:.1f})")

# Test
root = tk.Tk()
root.title("Debug Projection Test")
root.geometry("800x600")

canvas_3d = TestCanvas(root)
canvas_3d.pack(fill=tk.BOTH, expand=True)

print("If you see a blank window, check the console output above for projection issues")
root.mainloop()
